...
# Административный перечень субъектов РФ (код · название).
# Это справочные данные — при необходимости отредактируй состав под свою логистику
# (какие регионы реально обслуживаешь).
REGIONS = [
...
def unseed(apps, schema_editor):
    Region = apps.get_model("catalog", "Region")
    Region.objects.filter(code__in=[c for c, _ in REGIONS]).delete()
